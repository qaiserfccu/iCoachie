...
import { headCoachService, StaffMember, Meeting } from "@/lib/services/headCoachService"
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true)
        setError(null)
        const staffData = await headCoachService.getStaffMembers()
        setStaff(staffData.length > 0 ? staffData : fallbackStaff)
      } catch (err) {
        console.error('Failed to fetch staff:', err)
        setError('Failed to load staff. Using fallback data.')
