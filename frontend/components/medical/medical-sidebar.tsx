...
const menuItems = [
  { icon: LayoutDashboard, label: "Dashboard", href: "/medical" },
  { icon: FileText, label: "Health Records", href: "/medical/health-records" },
  { icon: AlertTriangle, label: "Injuries", href: "/medical/injuries" },
  { icon: Heart, label: "First Aid", href: "/medical/first-aid" },
]

const bottomItems = [
  { icon: Settings, label: "Settings", href: "/medical/settings" },
  { icon: Globe, label: "View Site", href: "/" },
  { icon: HelpCircle, label: "Help", href: "/medical/help" },
]
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-white/20">
          ...
          >
            <div className="w-10 h-10 bg-gradient-to-br from-red-500 to-pink-500 rounded-xl flex items-center justify-center flex-shrink-0 shadow-lg">
              <Stethoscope className="w-5 h-5 text-white" />
            </div>
            {!collapsed && (
              <div>
                <span className="text-lg font-bold text-foreground">iCoachie</span>
                <span className="block text-xs text-muted-foreground">Medical Staff</span>
              </div>
            )}
          </Link>
          <button
          ...
          {menuItems.map((item) => (
            <Link
              ...
              className={cn(
                ...
                isActive(item.href)
                  ? "bg-gradient-to-r from-red-500/20 to-pink-500/20 text-red-500 border border-red-500/20"
