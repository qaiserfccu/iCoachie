...
const menuItems = [
  { icon: LayoutDashboard, label: "Dashboard", href: "/venue" },
  { icon: CalendarDays, label: "Venue Bookings", href: "/venue/bookings" },
  { icon: PartyPopper, label: "Event Management", href: "/venue/events" },
  { icon: Users, label: "Capacity Planning", href: "/venue/capacity" },
  { icon: Star, label: "Amenities", href: "/venue/amenities" },
  { icon: DollarSign, label: "Pricing", href: "/venue/pricing" },
]

const bottomItems = [
  { icon: Settings, label: "Settings", href: "/venue/settings" },
  { icon: Globe, label: "View Site", href: "/" },
  { icon: HelpCircle, label: "Help", href: "/venue/help" },
]

export function VenueSidebar() {
  const [collapsed, setCollapsed] = useState(false)
  const pathname = usePathname()
  const isActive = (href: string) => pathname === href || (href !== "/venue" && pathname.startsWith(href))
    ...
    <>
      <aside className={cn("fixed left-0 top-0 z-40 h-screen glass-card border-r border-white/20 transition-all duration-300 hidden lg:flex flex-col", collapsed ? "w-20" : "w-72")}>
        <div className="flex items-center justify-between p-4 border-b border-white/20">
          <Link href="/venue" className={cn("flex items-center gap-2", collapsed && "justify-center")}>
            ...
            {!collapsed && (
              <div>
                ...
                <span className="block text-xs text-muted-foreground">Venue</span>
              </div>
            )}
          </Link>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 glass border-t border-white/20">
        <div className="flex items-center justify-around py-2">
          {menuItems.slice(0, 5).map((item) => (
            <Link key={item.href} href={item.href} className={cn("flex flex-col items-center gap-1 px-3 py-2 rounded-lg text-xs", isActive(item.href) ? "text-violet-400" : "text-muted-foreground")}>
