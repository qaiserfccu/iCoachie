...
const menuItems = [
  { icon: LayoutDashboard, label: "Dashboard", href: "/cleaning" },
  { icon: Calendar, label: "Schedules", href: "/cleaning/schedules" },
  { icon: Package, label: "Supplies Inventory", href: "/cleaning/supplies" },
  { icon: ClipboardCheck, label: "Inspections", href: "/cleaning/inspections" },
  { icon: Users, label: "Staff Assignments", href: "/cleaning/assignments" },
  { icon: FileText, label: "Quality Reports", href: "/cleaning/reports" },
]

const bottomItems = [
  { icon: Settings, label: "Settings", href: "/cleaning/settings" },
  { icon: Globe, label: "View Site", href: "/" },
  { icon: HelpCircle, label: "Help", href: "/cleaning/help" },
]

export function CleaningSidebar() {
  const [collapsed, setCollapsed] = useState(false)
  const pathname = usePathname()
  const isActive = (href: string) => pathname === href || (href !== "/cleaning" && pathname.startsWith(href))
    ...
    <>
      <aside className={cn("fixed left-0 top-0 z-40 h-screen glass-card border-r border-white/20 transition-all duration-300 hidden lg:flex flex-col", collapsed ? "w-20" : "w-72")}>
        <div className="flex items-center justify-between p-4 border-b border-white/20">
          <Link href="/cleaning" className={cn("flex items-center gap-2", collapsed && "justify-center")}>
            <div className="w-10 h-10 bg-gradient-to-br from-sky-500 to-cyan-500 rounded-xl flex items-center justify-center flex-shrink-0 shadow-lg">
              <Sparkles className="w-5 h-5 text-white" />
            </div>
            {!collapsed && (
              <div>
                <span className="text-lg font-bold text-foreground">iCoachie</span>
                <span className="block text-xs text-muted-foreground">Cleaning</span>
              </div>
            )}
          </Link>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 glass border-t border-white/20">
        <div className="flex items-center justify-around py-2">
          {menuItems.slice(0, 5).map((item) => (
            <Link key={item.href} href={item.href} className={cn("flex flex-col items-center gap-1 px-3 py-2 rounded-lg text-xs", isActive(item.href) ? "text-sky-400" : "text-muted-foreground")}>
