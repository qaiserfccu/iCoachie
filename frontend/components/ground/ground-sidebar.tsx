...
const menuItems = [
  { icon: LayoutDashboard, label: "Dashboard", href: "/ground" },
  { icon: Calendar, label: "Ground Schedules", href: "/ground/schedules" },
  { icon: Thermometer, label: "Field Conditions", href: "/ground/conditions" },
  { icon: Cloud, label: "Weather Monitoring", href: "/ground/weather" },
  { icon: BarChart3, label: "Usage Reports", href: "/ground/usage" },
  { icon: Wrench, label: "Maintenance", href: "/ground/maintenance" },
]

const bottomItems = [
  { icon: Settings, label: "Settings", href: "/ground/settings" },
  { icon: Globe, label: "View Site", href: "/" },
  { icon: HelpCircle, label: "Help", href: "/ground/help" },
]

export function GroundSidebar() {
  const [collapsed, setCollapsed] = useState(false)
  const pathname = usePathname()
  const isActive = (href: string) => pathname === href || (href !== "/ground" && pathname.startsWith(href))
    ...
    <>
      <aside className={cn("fixed left-0 top-0 z-40 h-screen glass-card border-r border-white/20 transition-all duration-300 hidden lg:flex flex-col", collapsed ? "w-20" : "w-72")}>
        <div className="flex items-center justify-between p-4 border-b border-white/20">
          <Link href="/ground" className={cn("flex items-center gap-2", collapsed && "justify-center")}>
            ...
            {!collapsed && (
              <div>
                ...
                <span className="block text-xs text-muted-foreground">Ground Staff</span>
              </div>
            )}
          </Link>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 glass border-t border-white/20">
        <div className="flex items-center justify-around py-2">
          {menuItems.slice(0, 5).map((item) => (
            <Link key={item.href} href={item.href} className={cn("flex flex-col items-center gap-1 px-3 py-2 rounded-lg text-xs", isActive(item.href) ? "text-emerald-400" : "text-muted-foreground")}>
