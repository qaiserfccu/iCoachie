...
const menuItems = [
  ...
  { icon: Users, label: "Team Management", href: "/head-coach/teams" },
  { icon: ClipboardList, label: "Training Plans", href: "/head-coach/training-plans" },
...
const bottomItems = [
  { icon: Settings, label: "Settings", href: "/head-coach/settings" },
  { icon: Globe, label: "View Site", href: "/" },
  { icon: HelpCircle, label: "Help", href: "/head-coach/help" },
]

export function HeadCoachSidebar() {
  const [collapsed, setCollapsed] = useState(false)
  const pathname = usePathname()
  const isActive = (href: string) => pathname === href || (href !== "/head-coach" && pathname.startsWith(href))
    ...
    <>
      <aside className={cn("fixed left-0 top-0 z-40 h-screen glass-card border-r border-white/20 transition-all duration-300 hidden lg:flex flex-col", collapsed ? "w-20" : "w-72")}>
        <div className="flex items-center justify-between p-4 border-b border-white/20">
          <Link href="/head-coach" className={cn("flex items-center gap-2", collapsed && "justify-center")}>
            ...
            {!collapsed && (
              <div>
                ...
                <span className="block text-xs text-muted-foreground">Head Coach</span>
              </div>
            )}
          </Link>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 glass border-t border-white/20">
        <div className="flex items-center justify-around py-2">
          {menuItems.slice(0, 5).map((item) => (
            <Link key={item.href} href={item.href} className={cn("flex flex-col items-center gap-1 px-3 py-2 rounded-lg text-xs", isActive(item.href) ? "text-orange-500" : "text-muted-foreground")}>
