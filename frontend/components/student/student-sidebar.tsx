...
import { LayoutDashboard, Calendar, TrendingUp, ClipboardCheck, MessageSquare, ChevronLeft, LogOut, HelpCircle, Settings, Globe, GraduationCap } from "lucide-react"

const menuItems = [
  { icon: LayoutDashboard, label: "Dashboard", href: "/student" },
  { icon: Calendar, label: "My Schedule", href: "/student/schedule" },
  { icon: TrendingUp, label: "My Progress", href: "/student/progress" },
  { icon: ClipboardCheck, label: "Evaluations", href: "/student/evaluations" },
  { icon: MessageSquare, label: "Message Coach", href: "/student/coaches" },
]
    ...
    <>
      <aside className={cn("fixed left-0 top-0 z-40 h-screen glass-card border-r border-white/20 transition-all duration-300 hidden lg:flex flex-col", collapsed ? "w-20" : "w-72")}>
        <div className="flex items-center justify-between p-4 border-b border-white/20">
          <Link href="/student" className={cn("flex items-center gap-2", collapsed && "justify-center")}>
            ...
            {!collapsed && (<div><span className="text-lg font-bold text-foreground">iCoachie</span><span className="block text-xs text-muted-foreground">Student Portal</span></div>)}
          </Link>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 glass border-t border-white/20">
        <div className="flex items-center justify-around py-2">{menuItems.map((item) => (<Link key={item.href} href={item.href} className={cn("flex flex-col items-center gap-1 px-3 py-2 rounded-lg text-xs", isActive(item.href) ? "text-indigo-500" : "text-muted-foreground")}><item.icon className="w-5 h-5" /><span className="truncate max-w-[60px]">{item.label}</span></Link>))}</div>
      </nav>
    </>
  )
}
