...
| Facilities / Venues / Grounds | `/api/facilities` | `controllers/facilityController.ts` | Facility CRUD, venue/ground nested CRUD, detail endpoints | Mix of `requireRole` + `requireScope`; club scoping enforced | See `docs/facilities/endpoint-matrix.md` for deep dive; missing metadata fields, staff assignment, availability, soft delete |
...
| RBAC Test Harness | `/api/rbac/test` | `controllers/rbacTestController.ts` | `GET /role`, `/scope`, `/permission` | Requires auth plus targeted middleware per route | Expand to cover new permissions once Card 24 lands |
...
- **Scope-based enforcement**: Facilities leverage `requireScope('FACILITY' | 'VENUE' | 'GROUND')`; other modules rarely use scopes even though roles define scope metadata. Future work (Card 24) will introduce fine-grained permission codes across modules.
...
| Gap Category | Description | Linked Card |
| --- | --- | --- |
| Metadata DTO coverage | CRUD payloads expose only a subset of Prisma fields (locations, amenities, billing, etc.) across multiple modules. | `19 - Metadata DTO expansion (global)` |
| Manager & staff assignment | Need consistent endpoints to assign managers/staff for clubs, facilities, venues, grounds, sessions. | `20 - Manager & staff assignment (global)` |
| Scheduling & availability | Sessions, venues, grounds, and bookings lack unified schedule/availability APIs. | `21 - Scheduling & availability (global)` |
| Soft delete & audit | Most controllers perform hard deletes despite schema-level `deletedAt`. Need global soft-delete + restore endpoints and audit visibility. | `22 - Soft delete & restore (global)` |
| Pagination & filtering | List endpoints return entire datasets with no pagination/search/sort semantics. | `23 - Pagination & filtering (global)` |
| Permission-driven RBAC | Only a few routes leverage `requirePermission`; need cross-module permission enforcement. | `24 - Permission-based RBAC (global)` |
